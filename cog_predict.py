...
FYI: You can run `python cog_demo` to download/pre-cache model data.
You can do that via `cog run python cog_demo` or just run it
...
from cog_prepare import run_alignment_offline, Preparer
...
class Predictor(cog.Predictor):
    ...
    def predict(self, input, style):
        ...
        res = Image.fromarray(np.array(final_res), mode="RGB")
